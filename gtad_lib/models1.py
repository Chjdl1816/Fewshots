...
def reshape_feature(base_feature):
        ...
        for i in range(len(base_feature)):
            if out_x is None:
                ...
                # 如果总共需要填充的长度是奇数，则将左侧填充长度加 1
                # if (left_pad + right_pad) % 2 == 1:
                #     left_pad += 1

                # 使用更新后的左、右填充长度进行填充操作（注意第 1 个维度已经被忽略）
                base_feature_after = F.pad(base_feature[i], [left_pad, right_pad], value=0)
                print(out_x.shape,base_feature_after.shape)

                out_x += base_feature_after
                print(base_feature_after.shape)
        norm = LayerNorm(out_x.shape[1])
        return norm(out_x.cuda())
...
class GTAD(nn.Module):
    def __init__(self, opt):
        ...
        self.backbone1 = nn.Sequential(
            GCNeXt(self.feat_dim, self.h_dim_1d,self.h_dim_1d,k=[3,3,3,3,3,3], groups=32, idx=self.idx_list),
        )

        # Regularization
        self.regu_s = nn.Sequential(
            GCNeXt(self.h_dim_1d, self.h_dim_1d,self.h_dim_1d, k=[3,3,3,3,3,3],groups=32),
        ...
        self.backbone2 = nn.Sequential(
            GCNeXt(self.h_dim_1d, self.h_dim_1d,self.h_dim_1d,k=[3,3,3,3,3,3], groups=32, idx=self.idx_list),
        )#改
    ...
    def forward(self, snip_feature):
        ...
        # base_feature = self.reshape_feature(base_feature)
        gcnext_feature = self.backbone2(base_feature)  #
        # gcnext_feature = self.reshape_feature(gcnext_feature)
    ...
    def reshape_feature(base_feature):
        ...
        for i in range(len(base_feature)):
            if out_x is None:
                ...
                out_x += F.pad(base_feature[i], (out_x[0].shape[0],out_x[0].shape[0],out_x[0].shape[0]), 'constant', 0)
        norm = LayerNorm(out_x.shape[1])
        return norm(out_x)
