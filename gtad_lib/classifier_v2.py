    ...
    def TransformerRePRI(self,features_s,features_q, gt_q , gt_s,count,mode):
        if mode =="train":
            ...
            if opt["cross_domain"]:
                ...
                ckpt = torch.load("output/Transformer_checkpoint_shot_"+str(opt["shot"])+".pth.tar")
           ...
           # viusalize(self.W_.view(1,256),query_fg.view(t,d),gt_q.view(100)[fg_ind],count,modes="before")#注释掉
            # viusalize(torch.mean(features_s.view(5,100,256),0),features_q.view(100,256),gt_q.view(100))
            self.prototype_updated = (self.self_attn(self.W_.unsqueeze(1), feat_q,feat_q)).squeeze(1)
            #viusalize(self.prototype_updated.view(1,256),query_fg.view(t,d),gt_q.view(100)[fg_ind],count,modes="after")#注释掉
            ...
            if(torch.isnan(torch.min(proba_q))):
                print('11111')
            if self.use_trans:
